...
-- 13. 毛玻璃（终端背景模糊）
--
--     分工：模糊是【合成器】做的，终端只负责「变透明」。
--       ▸ kitty 侧  background_opacity 0.6（config/kitty/kitty.conf）
--                   —— 只是让窗口带 alpha 通道，kitty 自己没有任何模糊能力。
--       ▸ Hyprland  检测到窗口有 alpha，就对它背后的合成结果跑双 Kawase 模糊。
--     所以把 background_opacity 改回 1，模糊会整个消失 —— 合成器无处可糊。
--
--     ⚠️ kitty 有个 background_blur 选项，要求合成器实现 KDE blur 扩展协议，
--     Hyprland 不实现（走自己的 alpha 检测路径），在这台机器上设了也没用。
--
--     另见 config/windowrules.lua 的终端 opacity 规则（"1.0 override"）：
--     它让 Hyprland 放弃对终端施加 active/inactive_opacity(0.95/0.85)，
--     透明度完全交给 kitty 的 0.6 —— 这就是 kitty 聚焦/失焦不变明暗的原因。
--
--     官方 config/decorations.lua 是 CachyOS 原版（与 /etc/skel 逐字节相同），
--     下面这些值原本全来自它。搬到这里是为了「改观感不用动官方文件」——
--     hl.config 逐项合并，只写 blur 不会冲掉它的 rounding / *_opacity。
--     （同第 6b、11 节验证过的合并行为。）
--
--     ★ 策略：【糊透 + 微压暗】—— 这是一组**通用**值，换壁纸不用重调。
--     2026-08-28 用 grim 逐版截图比出来的，理由逐行写在下面。
--
--     调参手册：括号里是 Hyprland 默认值，验证一律看
--     `hyprctl getoption decoration:blur:<键>` 的 set: true —— set: false 是在报默认值。
--     实时试参数：hyprctl eval 'hl.config({ decoration = { blur = { size = 20 } } })'，
--     hyprctl reload 还原。改完想看效果就 grim 截一张，别靠肉眼记忆比。
-- ────────────────────────────────────────────────────────────────────────────
hl.config({
    decoration = {
        blur = {
            -- ★ 为什么是「糊透」而不是「糊一半」
            --
            -- Kawase 模糊每加一 pass 就把画面降采样一半，passes=4 等于降到 1/16 ——
            -- 壁纸里任何内容都会被化成纯色块，认不出原图。这正是要的：
            -- **认不出 = 永远不会有一块亮东西压在代码上**。
            --
            -- 走过的弯路（别再试一遍）：曾经调到 passes=1 + size=13，想让壁纸
            -- 主体的剪影柔和透出来。对着稀疏的 ASCII art 壁纸确实好看，但换成
            -- 正常插画（实心、高对比）立刻翻车 —— 一张脸清清楚楚压在文字上，
            -- 亮块直接盖住整段代码。**保留结构的方案没法通用，糊透的才行。**
            --
            -- 所以氛围感不靠「透出图案」，靠下面的 vibrancy 留住色调。
            -- 代价是壁纸再好看也只贡献一团光晕，那是通用性换来的，值。
            passes = 4,

            -- 采样半径。passes 已经糊透了，size 只决定光晕摊开的范围。
            -- 6~10 差别很小，8 是中庸值。加大不会更糊，只会更吃 GPU。
            size = 8,

            -- 对 special workspace（本配置的抽屉架，见第 10 节）也模糊。
            special = true,

            -- 磨砂颗粒。大面积纯深色模糊会起色带(banding)，一点噪点就能压掉，
            -- 顺便给玻璃面一点材质感。0.06 试过，会显脏。
            noise = 0.025,  -- (0.0117)

            -- 保持中性。>1 会把糊剩的边缘重新拉出来，等于抵消糊透的努力，
            -- 还会把噪点放大成斑驳。别动。
            contrast = 1.0,  -- (0.8916)

            -- ★ 氛围感全靠这一项。默认 0.17 会把壁纸洗成灰，0.5 能留住紫/蓝调，
            -- 和 tokyonight 的冷调对得上 —— 让背景「有色但无形」。
            vibrancy = 0.5,  -- (0.1696)

            -- 微压暗，给可读性留余量。深色壁纸配浅色主体时，主体糊完仍可能偏亮，
            -- 0.9 保证任何壁纸都不会亮到影响读代码。这是通用性的保险丝。
            brightness = 0.90,  -- (1.0)

            -- ▼ 以下保持默认，理由记在这儿免得下次重新试一遍 ▼

            -- 只模糊壁纸不模糊下层窗口。本机终端常全屏，底下本来就只有壁纸，
            -- 开了看不出差别，却会让浮动窗口叠加时透不出下层。不值得。
            -- xray = true,   -- (false)

            -- 对弹出层(popup)也模糊。kitty 没有 popup，对终端无影响。
            -- popups = true,  -- (false)
        },
    },
})
...
-- 14. 文件管理器：ALT + E
--
--     官方 config/binds.lua 第 72 行绑的是 SUPER + E，命令是
--     `uwsm app -- dolphin`（launchPrefix .. FILE_MANAGER）。这里只换修饰键，
--     命令原样不动 —— 和第 12 节（启动器换 ALT+Space）是同一个套路。
--     unbind 在第 1 节。
--
--     为什么：本配置的「动作类」键位已经全部收在 ALT 上（Enter 全屏、
--     [ / ] 切桌面、\ 分屏、T 新桌面、W 关窗、S 抽屉、Space 启动器、9 顶栏），
--     开文件管理器同属这一类，跟着回 ALT 才不用在两组修饰键之间来回猜。
--     字母 E 保留不变，只是小指 SUPER 换成拇指 ALT，肌肉记忆不用重学。
--
--     FILE_MANAGER 走 config/variables.lua 的全局值（当前 dolphin），
--     换文件管理器改那一处即可，这行不用动。launchPrefix 是本文件第 5 节
--     定义的 file-local，作用域覆盖到这里，和官方 binds.lua 的值一致。
--
--     ⚠️ 同第 8、12 节的老问题：ALT + E 在带菜单栏的 GTK/Qt 应用里原本是
--     「Edit」菜单的助记键，Wayland 下合成器全局 bind 会先截获，应用收不到。
--     日常影响很小（这类应用基本都有别的入口），Wine 程序里会明显一点。
-- ────────────────────────────────────────────────────────────────────────────
hl.bind("ALT + E", hl.dsp.exec_cmd(launchPrefix .. (FILE_MANAGER or "dolphin")))


-- ────────────────────────────────────────────────────────────────────────────
-- 15. 屏幕取字（OCR）：SUPER + SHIFT + O 框选 ／ SUPER + ALT + O 整屏
--
--     把屏幕上的文字识别出来直接进剪贴板。替代 normcap —— 它底层是 Tesseract，
--     对屏幕小号中文粘连、漏字，且每次按键都要重新加载模型。这里换成
--     RapidOCR（PP-OCRv6 + onnxruntime），并把模型加载抽进常驻服务。
--
--     ★ 为什么这一节用 SUPER 而不是 ALT —— 本文件其余「动作类」键位全在 ALT 上
--       （见第 14 节的清单），这里是【有意的例外】：取字属于「截图家族」，
--       而这一家子的修饰键分工是官方 config/binds.lua 定下的另一套 ——
--           SHIFT = 框选，ALT = 整屏
--           截图 SUPER+SHIFT/ALT + P   录屏 SUPER+SHIFT/ALT + R
--       取字是这家子的第三个成员，跟着它们走 O 才不用记两套规则。
--       手指记忆是「同一个位置换个字母」，比归队到 ALT 更省脑子。
--
--     O 键在【所有】修饰键组合下都没被占用（hyprctl binds 核实过），
--     纯新增，不需要 hl.unbind。
--
--     不加 launchPrefix，理由和官方 binds.lua 里录屏那三行同源但不同：
--     录屏是怕 uwsm 的独立 cgroup 让 pidfile 追不上；这里是因为 ocr-grab 要用
--     wl-copy 写剪贴板、notify-send 发通知，直接继承 Hyprland 会话的
--     WAYLAND_DISPLAY / DBUS_SESSION_BUS_ADDRESS 最省事，而且它是个几百毫秒
--     就结束的短命进程，套一层 uwsm 只有开销没有收益。
--
--     ⚠️ 识别服务 ocrd.service 是 systemd socket 激活的，空闲 10 分钟自己退出
--       把内存还回去。所以【每天第一次】按这个键会等 2~3 秒（现拉进程 + 加载
--       模型），之后都是几十毫秒。这不是卡住了。
--       想改成开机常驻不退出：把 ocr-server 里的 IDLE_TIMEOUT 调大即可。
--       原理、调参入口和踩过的坑见 docs/10-ocr.md。
-- ────────────────────────────────────────────────────────────────────────────
